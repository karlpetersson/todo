...
static int __parse_style(char *style_raw, cbuf_t *result) {
	...
	while(*ptr) {
		if(*ptr == '$' && *(ptr + 1) == '(') {
			...
			if(*ptr == 'c' || *ptr == 'C') {
				if(!(mod = co_from_id(*(++ptr) - '0'))) return TODO_ESTL_CRNUM;
				cbuf_puts(result, mod, strlen(mod));

			} else if(*ptr == 'b' || *ptr == 'B') {
				if(!(mod = bg_from_id(*(++ptr) - '0'))) return TODO_ESTL_BGNUM;
				cbuf_puts(result, mod, strlen(mod));

			} else if(*ptr == 's' || *ptr == 'S') {
				if(!(mod = st_from_id(*(++ptr) - '0'))) return TODO_ESTL_SPNUM;
				cbuf_puts(result, mod, strlen(mod));
				...
				if(fcnt > 2) return TODO_ESTL_WFRMT;
				found[fcnt++] = 'N';
	...
	if(fcnt < 3)
		return TODO_ESTL_VMSNG;

	if(!(found[0] == 'N' && found[1] == 'P' && found[2] == 'T'))
		return TODO_ESTL_WORDR;
...
int styles_from_json(Style_t *style, const char *path) {
	...
	if((active_style = cJSON_GetObjectItem(styles, active_style_id)) == NULL) {
		print_user_error(TODO_ESTL_NOACTV, active_style_id);
		return TODO_ESTYLE;
	} 
